...
void TextureLoader::Destroy() {
	...
	//--- !NULLでそのポインタが指すオブジェクトを削除、ポインタを NULL に設定。
	//--- これは通常、シングルトンデザインパターンにおけるインスタンスの解放（メモリ解放）のために使用される
	if (m_pInstance != NULL) {
		delete m_pInstance;
		m_pInstance = NULL;
	}
...
//      S_OK : succeeded
//
HRESULT TextureLoader::CreateD2D1BitmapFromFile(ID2D1RenderTarget* pRenderTarget, const TCHAR* pFilename, ID2D1Bitmap** ppBitmap) {
	HRESULT hr;
	...
	if (len > 3) {
		TCHAR* pTale = (TCHAR*)pFilename;
		pTale = (TCHAR*)pFilename + (len - 4);
#ifdef _TARGA_SUPPORTED_
		if (0 == _tcsicmp(pTale, _T(".tga"))) {
			return CreateD2D1BitmapFromTgaFile(pRenderTarget, pFilename, ppBitmap);
		}
#endif
#ifdef _DDS_SUPPORTED_
		if (0 == _tcsicmp(pTale, _T(".dds"))) {
			//  create texture from dds file
			return CreateD2D1BitmapFromDdsFile(pRenderTarget, pFilename, ppBitmap);
		}
#endif
	}

	//  create texture using WIC
	TextureLoader* pInstance = GetInstance();

#ifndef UNICODE
	{
		size_t dstlen;
		mbstowcs_s(&dstlen, NULL, 0, pFilename, _TRUNCATE);

		LPWSTR filename = new WCHAR[dstlen + 1];
		mbstowcs_s(&dstlen, filename, dstlen, pFilename, _TRUNCATE);
		hr = pInstance->m_pFactory->CreateDecoderFromFilename((LPCWSTR)filename, 0,
	...
	if (pixelFormat != GUID_WICPixelFormat32bppBGRA) {
		...
		hr = pRenderTarget->CreateBitmap(sizBitmap, bitmapProperties, &pOutput);
		if (SUCCEEDED(hr)) {
			...
			pOutput->CopyFromMemory(NULL, pBuffer, width * 4);
			delete pBuffer;
		}
		else {
		...
		if (SUCCEEDED(hr)) {
			...
			double frac = 1.0 / 255.0;
			double a;
			pFrame->CopyPixels(NULL, width * 4, width * 4 * height, pBuffer);
			for (int row = 0; row < height; ++row) {
				BYTE* p = pBuffer + (width * 4) * row;
				for (int col = 0; col < width; ++col) {
					a = frac * p[3];
...
//      S_OK : succeeded
//
HRESULT TextureLoader::CreateTextureFromFile(ID3D11DeviceContext* pContext, const TCHAR* pFilename, ID3D11Texture2D** ppTexture, DWORD* pSrcWidth, DWORD* pSrcHeight, FillMode dwFillmode) {
	IWICBitmapDecoder* pDecoder = NULL;
	IWICBitmapFrameDecode* pFrame = NULL;
	WICPixelFormatGUID pixelFormat;
