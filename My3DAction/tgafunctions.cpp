...
//
HRESULT CreateTextureFromTgaFile(ID3D11DeviceContext* pContext, const TCHAR* pFilename, ID3D11Texture2D** ppTexture, DWORD* pSrcWidth, DWORD* pSrcHeight, FillMode dwFillmode) {
	FILE* fp = NULL;
	errno_t err;
...
HRESULT CreateD2D1BitmapFromTgaFile(ID2D1RenderTarget* pRenderTarget, const TCHAR* pFilename, ID2D1Bitmap** ppBitmap) {
	FILE* fp = NULL;
	errno_t err;
	unsigned char header[256];
	DWORD srcwidth, srcheight;
	...
	if (fp != NULL) {
		...
		if (flag & 0x30) {
			...
			double a, frac = 1.0 / 255.0;
			for (int y = 0; y < (int)srcheight; ++y) {
				pSource = (((BYTE*)pBuffer) + sourceY * row_pitch);
				pSource += sourceX << 2;
				pDest = (((BYTE*)pBuffer2) + y * row_pitch);
				for (int x = 0; x < (int)srcwidth; ++x) {
					a = frac * pSource[3];
					//    pre-multiply alpha, because d2d requres
					//  premultiplied rgb value to bitmaps to be drawn.
					pDest[0] = (BYTE)(a * pSource[0]);	//  B
					pDest[1] = (BYTE)(a * pSource[1]);	//  G
					pDest[2] = (BYTE)(a * pSource[2]);	//	R
					pDest[3] = (BYTE)pSource[3];		//  A
					pDest += 4;
					pSource += diffX * 4;
				}
				sourceY += diffY;
